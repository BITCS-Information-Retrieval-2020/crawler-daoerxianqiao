...
class ErqiaocrawlerPipeline:
    def process_item(self, item, spider):
        return item


class CrossmindPipeline:
    ...
    def process_item(self, item, spider):
        ...
        # 视频本地路径
            ...
            self.db[self.collection[0]].update({'foreign_id': item['target_id']}, {'$set': {'video_path': item['video_path']}})
        # pdf本地路径
        elif 'pdf_path' in item:  # 注意 在acl中的视频路径是Video_path
            self.db[self.collection[0]].update({'foreign_id': item['target_id']}, {'$set': {'pdf_path': item['pdf_path']}})
        # 评论信息
            ...
            for k, v in item.items():
                if k != 'Anthology ID':
                    if k == 'PDF_path':
                        ...
                        self.db[self.collection[3]].update_one({'Anthology ID': item['Anthology ID']}, {'$addToSet': {k: v}})
        return item
